...
function PracticeWorkspace({ problem, onClose, onSave }) {
  ...
  return <div className="practice-backdrop">
    <section className="practice-workspace" aria-label="Practice workspace">
      <header className="practice-header"><div className="breadcrumb"><button onClick={onClose}>Library</button><b>/</b><strong>Practice</strong></div><button className="drawer-close" onClick={onClose} aria-label="Close practice workspace">×</button></header>
      <nav className="practice-tabs" aria-label="Problem views">{[['description', 'Description'], ['hint', 'Hint'], ['solution', 'Solution']].map(([tab, label]) => <button key={tab} className={activeTab === tab ? 'active' : ''} onClick={() => setActiveTab(tab)}>{label}{tab === 'hint' && <span>1</span>}</button>)}</nav>
      <div className="practice-content">
        {activeTab === 'description' && <article className="problem-statement"><p className="eyebrow">PROBLEM / {String(problem.id).padStart(2, '0')}</p><span className="problem-type">{problem.type}</span><h1>{problem.title}</h1><p className="statement-label">Problem statement</p><p className="statement-copy">{problem.prompt || problem.summary}</p><LogicDiagram problem={problem} /><div className="statement-details"><div><span>Difficulty</span><strong className={`difficulty ${problem.difficulty.toLowerCase()}`}>{problem.difficulty}</strong></div><div><span>Expected time</span><strong>{problem.time}</strong></div><div><span>Skills</span><strong>{problem.skills.join(' · ')}</strong></div></div></article>}
        {activeTab === 'hint' && <article className="learning-panel"><p className="eyebrow">GUIDED REASONING</p><h1>Use the signal, not the shortcut.</h1><p className="learning-lead">A good hint should move your thinking one step forward without giving away the implementation.</p><div className="hint-card"><span>HINT 01</span><p>{problem.hint || 'Break the problem into named intermediate signals, then simplify one step at a time.'}</p></div><div className="learning-note"><strong>Try before revealing the solution</strong><span>Write down the inputs, outputs, and one intermediate condition in the playground.</span></div></article>}
        {activeTab === 'solution' && <article className="learning-panel"><p className="eyebrow">EDITORIAL / SIGNAL PATH</p><h1>How to approach it</h1><p className="learning-lead">Use this path to review the reasoning after you have made an attempt.</p><div className="solution-steps">{solutionSteps.map((step, index) => <div key={step}><b>{String(index + 1).padStart(2, '0')}</b><p>{step}</p></div>)}</div><div className="answer-reveal"><span>REFERENCE ANSWER</span><code>{problem.answer || 'No reference answer published yet.'}</code></div></article>}
        <section className="answer-panel playground-panel"><div className="playground-kicker"><span className="playground-dot" />Live playground <span className="editor-mode">Digital logic</span></div><div className="answer-panel-heading"><div><p className="eyebrow">YOUR RESPONSE</p><h2>Work it through</h2></div><span className="practice-status">{submitted ? (isCorrect ? 'Correct' : 'Try again') : 'Not submitted'}</span></div><div className="editor-toolbar"><span>Expression editor</span><button onClick={() => { setAnswer(''); setSubmitted(false); setRunStatus(''); }}>Reset</button></div><textarea value={answer} onChange={(event) => { setAnswer(event.target.value); setSubmitted(false); setRunStatus(''); }} placeholder="Type your Boolean expression, value, or reasoning here..." aria-label="Your answer" />{showHint && <div className="hint"><span>i</span><p>{problem.hint || 'Break the problem into named intermediate signals, then simplify one step at a time.'}</p></div>}<div className="test-cases"><span>Test cases</span><button className="case active">Case 1</button><button className="case">Case 2</button><button className="case">Case 3</button></div><div className="answer-actions"><button className="hint-button" onClick={() => { setShowHint((visible) => !visible); setActiveTab('hint'); }}>{showHint ? 'Hide hint' : 'Need a hint?'}</button><div className="run-submit"><button className="run-answer" onClick={() => setRunStatus(answer.trim() ? 'Expression ready to evaluate.' : 'Enter an answer first.')}>Run</button><button className="submit-answer" onClick={() => { setSubmitted(true); setRunStatus(''); }}>Submit <span>→</span></button></div></div>{runStatus && <div className="run-status">{runStatus}</div>}{submitted && <div className={`feedback ${isCorrect ? 'correct' : 'incorrect'}`}><strong>{isCorrect ? 'Signal confirmed.' : 'Not there yet.'}</strong><span>{isCorrect ? 'You can move on or save this result to your progress.' : 'Compare your structure with the hint and try one more time.'}</span></div>}</section>
      </div>
      <footer className="practice-footer"><span>Digital logic / {problem.skills.join(' · ')}</span><button onClick={() => onSave(problem)}>☆ Save to practice queue</button></footer>
    </section>
  </div>;
}
...
function App() {
  ...
  const chooseDomain = (nextDomain) => { setDomain(nextDomain); setActiveView('library'); };
